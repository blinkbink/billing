...
public class UpdateMinimumNotifBalance extends ActionSupport {
...
//		}
	  	...
	  	if(!jsonRecv.has("active")) 
	  	{
		  		notif="JSON active does not exist";
		  	  	jo.put("result", "28");
				jo.put("notif", notif);
				jo.put("log", uuid);
				jo.put("ptime", formatter.format((System.nanoTime() - startTime)/1000000000d));
  	  	...
  	  	try {
				...
				{
					account.setNotif_balance(new BigDecimal(jsonRecv.getString("amount")));
					account.setSend_notif_balance(jsonRecv.getBoolean("active"));
					db.session().update(account);
					
					try
					{
						LogSystem.info(request, "Try update minimum balance notification", uuid);
						tx.commit();
						LogSystem.info(request, "Success update minimum balance notification", uuid);
					}catch(Exception e)
					{
						LogSystem.error(getClass(), e, uuid);
					}
					...
					jo.put("result", "00");
		  	  		jo.put("notif", "Success update minimum balance notification");
					jo.put("log", uuid);
					jo.put("ptime", formatter.format((System.nanoTime() - startTime)/1000000000d));
		  	  		LogSystem.info(request, jo.toString(), uuid);
		  	  		return jo;
